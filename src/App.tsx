...
export default function App() {
  ...
  // Carrega Departamentos Permitidos baseado no E-mail
  useEffect(() => {
    if (!session) return;
      ...
      if (data) {
        ...
        if (isAdmin) {
          ...
          if (myConfig && myConfig.departamentos && myConfig.departamentos.length > 0) {
            ...
            setAllowedDepts([]); // Sem acesso a nenhum departamento
          }
        }
      }
  ...
  if (authLoading || !settingsLoaded) return <div className="min-h-screen bg-slate-50 flex items-center justify-center font-bold text-[#2563eb]">Carregando acessos do sistema...</div>;
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4">
        <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full text-center border border-red-100">
          ...
          <p className="text-slate-600 mb-6">Seu e-mail (<b>{userEmail}</b>) ainda não foi vinculado a nenhum departamento nas configurações.</p>
  ...
  return (
    <div className="flex h-screen bg-slate-100 overflow-hidden font-sans">
      ...
      <div className={`${isSidebarCollapsed ? 'w-20' : 'w-64'} bg-white border-r border-slate-200 flex flex-col shadow-sm z-50 transition-all duration-300 ease-in-out relative`}>
        ...
        {/* SELETOR DE DEPARTAMENTO DINÂMICO */}
        {!isSidebarCollapsed && allowedDepts.length > 0 && (
